...
ADR-0005 §四把 LLM 语义润色定为未来特性，并划出两段式边界；ADR-0006 §四又把它从 warning 触发中拆出来。但两份 ADR 都只写了「不做什么」：不进 required check、不拿模型当 CI 判定器、不接进 `--fix`。真要动手，还有一层问题没有答案：这个能力叫什么、怎么进命令行、调谁的模型、探不到引擎时怎么办、模型把标题改坏了谁来拦、prompt 长什么样、先做哪一步。

用户 2026-08-31 至 09-01 与 orchestra 的设计讨论逐条定案，本 ADR 是这次讨论的正式记录。证据链有两条：

- `docs/research/llm-polish-survey.md` —— 两个 claudish 项目的源码级机制 (Gvozdev 的 hook + fail-open + 可整体替换的 prompt 文件、Deng 的 205 行分层 spec 与「词典不进 prompt、禁止机械换词」)，以及由此得出的形态建议与不建议做的事。
- **本机实测**，2026-09-01 在开发机上直接跑三家 CLI 得到的事实，下文逐条标注。实测是这批决定的主要依据 —— 用户的原话是「实践是检验真理的唯一标准」。
...
工具名、binary、包名、crate 名统一为 **`limae`**。取自贺拉斯 (Horace) 的 *limae labor*「锉刀之功」 —— 反复打磨文字，正是这三个子命令共同的隐喻。

**占用核实**，决策时点 (2026-09-01) 的结果 —— 它是选这个名字的依据，不随后续迁移改写：crates.io 明确回 `crate limae does not exist`，PyPI 与 npm 均 404，GitHub 仓名 `Luolc/limae` 可用；GitHub 用户名 `limae` 已被占用，只影响将来是否能起同名组织，不影响仓名与包名。

**避让**：与重点参考对象 AutoCorrect、zhlint 没有字面或语义冲突；名字里不含 `lint`、`correct`、`md`，不把范围限制在「Markdown 的 linter」上 —— `polish` 一旦成立，这个工具就不只是 linter 了。

**迁移范围**，同时是一份对账表 —— 除 GitHub 仓名外，本 ADR 都只列清单、不执行：
...
GitHub 仓库已于 2026-09-01 由 orchestra 在本 ADR 合并前改名：`Luolc/lo-md-lint` → `Luolc/limae`，旧 URL 由 GitHub 301 永久重定向，本地 origin 已同步。**本地目录改名与消费仓迁移仍未做** —— 前者等所有 agent 收工再做，后者由 butler 另派任务。

**过渡期保留旧名别名**：命令、配置表与文件名、hook id、行内指令前缀、忽略文件都要在一段时间内同时支持新旧名称，消费仓 (`machine-setup` / `wealth-management` / `butler`) 各出一个小 PR 跟进。旧别名何时移除由后续任务决定，本 ADR 只要求「有过渡期」，不定期限。
...
三档分别是「只报 / 机械改 / 模型改」，风险递增，用户一眼就能分清谁动了自己的文字。这也是两段式边界 (ADR-0005 §四) 在命令行上的体现：`check` 与 `format` 是确定性段，`polish` 是语义段，两段各占一个子命令、不互相触发。
...
`claude` / `codex` / `grok` 三个预设各是一份内置的**命令模板展开** —— 不自建 HTTP 请求、不自己解析各家凭证，只调本机已登录的 CLI。这一条来自调研：Gvozdev 的 `providers.sh` 自建四种 provider 的请求，连带把 oauth、Keychain、ambient key 的整个攻击面搬进了工具 (`llm-polish-survey.md` §5.2 已判断本仓不该重复它)。

三份模板把本机实测发现的问题固化进去 (实测日期 2026-09-01)：
...
| `grok` | `grok --system-prompt-override <spec> -m <model> -p <正文>` | `--system-prompt-override` (别名 `--system-prompt`) 与 `--rules` 都能生效，**但只在 4.6 上**；`grok-4.5` 会无视覆盖、继续用自带的 agent 人格并带上仓库上下文 |
...
新增一个预设，或更换某个预设的默认型号，都要先过一条最小验收：用一条**只可能来自我们 spec 的指令** (例如「只回一个词 PONG」) 去问，看这个型号能否压住自己的内置人格与项目上下文。**不通过的型号不进预设表。**

这条来自 grok-4.5 的问题 (§三 实测)：同一份覆盖参数，4.5 两种写法都无视、继续用自带人格，4.6 两种都听。测法便宜、可复现、与具体 prompt 内容无关，所以写进本 ADR 作为常设验收，而不是留在某次调试记录里。
...
**社区证据** (research agent 2026-09-01 回报，只回报未入库；以下是**社区评价，非本仓实测**)：便宜档的散文质量普遍下降 —— Luna 在两个盲评里明显低于 Sol 与 Terra；Haiku 4.5 被社区口径列为「会被人读的成稿别用」；grok-4.5 在写作榜垫底，且多处观察到人格回潮 (与 §三 的 PONG 实测同向)；`gpt-5.4-mini` 找不到任何写作专项评测。另外两条对设计直接有用：coding 特化模型的语言劣势主要是**对人偏好偏冷偏短**，不是爱列 bullet；OpenAI 称 5.6 默认已经更短、套话更少，**prompt 里再强调「简洁」会过头** —— 这一条要带进 §九 的 spec 迭代。

**暂定默认** (标明待 A/B 替换)：codex 用 `gpt-5.6-terra`，claude 用 `sonnet`，grok 用 `grok-4.6`。型号一律可配。

**判据与测法**：用本仓与消费仓的真实中文段落做 10–20 条盲对照，人评哪个更像人话；采集手段就是 §十 P0 的 hook A/B 模式。**降到便宜档必须有自家 A/B 证据，不能只凭公开评测或成本。**
...
这一条修正了 ADR-0005 §四「产物是旁路文件或建议」的形态判断 (边界本身不变：仍然人审后才算数、仍然不进 required check)。理由是仓库本来就在 git 里，`git diff` 就是天然的人审面；再造一个旁路目录，就得再管 `.gitignore`、再定义一个「接受」动作、再解释旁路文件与工作区谁是源文件，全是多余的概念。

### 八、正确性怎么守：结构确定性加语义模型裁判

两道关卡，性质不同。

**结构层，确定性比对，不花钱。** 改写前后**逐字不变**的至少要包括：围栏代码块、行内代码、链接目标与锚点、标题行、表格结构。任何一项变了，这次改写就判不合格 (按 §六 分场景报出或拒绝写入)。这挡住的是具体事故：**模型改了标题，别的文件对它的引用当场失效。** 同时 prompt 默认写明不要碰标题行 —— 标题是跨文件契约，散文才是润色对象。
...
一份 **general spec (英文)**，加每种语言各一份 **distilled spec，用该语言自己写**。中文那份先从 Deng 的 `specs/claudish-to-english.md` 翻译起步，再自行演进，不预先定死内容。
...
1. **P0：hook 里的一次输出润色** —— 局部文本、不写盘、没有跨文件问题，正好用来迭代 prompt。**A/B 模式挂在这里**：按概率同时给出改前与改后，人评之后收集反馈；其它仓可以挂同样的 hook，经 herdr 把反馈发回本仓。§五 的型号判据靠它采集。
...
- **更名迁移已于 2026-09-01 全部完成**：分发身份 (包名、命令名、仓库 URL) 见 PR #33 (版本 0.10.0)，用户可见的配置语法见 PR #35 (版本 0.11.0，已发 tag v0.11.0)。四个用户可见的名字 (`limae.toml`、`[tool.limae]`、`limae-disable` 一族、`.limae-ignore`) 与命令名、pre-commit hook id 均各留旧名别名，升级对既有消费方是 no-op。**§一 的迁移表不改** —— 它记的是决策时点的状态，执行进度以本条与 git 历史为准。
...
§十 P0 的「不写盘」一语由 ADR-0012 写清 (2026-09-01)：它指的是不动用户的文件、不产生需要人审的旁路产物，不是禁止会话态的临时状态。单路润色的会话态记录据此成立，落地顺序与其余各条不变。

§十 P0 (hook 里的一次输出润色、A/B 挂在这里) 由 [ADR-0016](0016-hook-mechanical-only.md) 取代 (2026-09-11)：hook 只做机械排版修复，不再调 polish。§五 的证据路径 (A/B 由 P0 的 hook 采集) 随之不复存在，暂定默认型号冻结、证据来源转为开放问题，处置见 ADR-0016「后果」第一条；§五 的判据本身与 §二、§三、§四、§六、§七、§八、§九 不变。

§七 (不做旁路目录) 的范围由 ADR-0009 §五 澄清 (2026-09-01)：它挡的是文档润色的产物，运行时的会话态调试 / 判据数据不在其内。这一句按 tracker 那条「将来有任务碰这份 ADR 时顺手补齐」的约定于 2026-09-11 补记。
