...
/// Every `kind` the hook can write has a row in the handbook.
///
/// The diagnostics file is only useful if a reader can look a `kind` up, so a
/// category that exists in the code and not in the table is a category nobody
/// can act on. This is a regression test rather than a habit: the table has
/// already drifted once.
///
/// The list below is exhaustive by construction — `position` is a match over
/// each enum, so a new variant stops this compiling until it is named here, and
/// the assertion then stops it until the handbook lists it too.
#[test]
fn the_handbook_lists_every_kind_the_hook_can_write() -> TestResult {
    ...
    assert_eq!(
        REASONS.map(reason_position).to_vec(),
        (0..REASONS.len()).collect::<Vec<_>>()
    );
    assert_eq!(
        OTHERS.map(kind_position).to_vec(),
        (0..OTHERS.len()).collect::<Vec<_>>()
    );

    let handbook = include_str!("../../docs/knowledge/polish-hook-self-trial.md");
    let rows: Vec<&str> = handbook
        .lines()
    ...
    let mut missing: Vec<&str> = REASONS
        .map(|reason| Kind::Engine(reason).as_str())
        .into_iter()
        .chain(OTHERS.map(Kind::as_str))
        .filter(|kind| !rows.contains(kind))
        .collect();
