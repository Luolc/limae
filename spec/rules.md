...
- **判定与报告**：由两个消费型模式组成。标点前模式匹配「非空白且不是 `—` `⸺` `|` 的字符 + 一段连续的空格 (U+0020) + 集合 `，` `。` `、` `；` `：` `？` `！` 里的全角标点」；标点后模式匹配次序相反的「集合内标点 + 一段连续的空格 + 非空白且不是 `—` `⸺` `|` 的字符」。两个模式各自从行首向行尾独立扫描，每个模式内只取从左到右的非重叠匹配；每次匹配消费空格串及两端字符，所以下一个匹配不能复用同一模式已消费的端点。同一段字符仍可由两个独立模式各匹配一次，例如 `， ，` 报两处。
