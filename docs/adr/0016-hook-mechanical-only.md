...
- **输入**：一条 `MessageDisplay` 事件。用到的字段只有 `hook_event_name`、`session_id`、`message_id`、`turn_id` (与 `message_id` 一起构成 §二 的消息实例键)、`index`、`final`、`delta`、`cwd`；其余字段读到也不用。
...
读数 B 说明：把一条消息拆成批次来修，与整篇修的分歧来自「过去决定现在」的状态 —— 样本里现形的是围栏代码块 (`spec/rules.md`「全局豁免」第 1 条)，规范里同类的还有跨行的行内指令、段落分组、跨行的行内代码 span。本 ADR 第一版只带一个「在不在围栏里」的布尔；审查方 (PR #178 两轮) 与 orchestra 各自独立指出它的问题：hook 自己数围栏行，就有了第二个「什么算代码」的真相来源，块模型一变 (缩进代码块、围栏长度匹配) 布尔就不够，而没有任何门会提醒；跨行指令与 span 更是一个布尔装不下的。**裁定改为 orchestra 提出的前缀重放**：hook 里不写任何解析器，一切「过去决定现在」的状态 (围栏、成对指令、`disable-next-line`、段落分组) 由 canonical pipeline 算，与 `limae --fix` 永远同源。

**「单一真相来源」是耦合性质，不是正确性保证，两者要分开写。** orchestra 最初的论据「crate 已有一套处理 `~~~`、围栏长度、缩进代码块的块解析器，复用它就免费拿到」**是假的**，经独立裁决方证伪、orchestra 读源码复核 (2026-09-11)：`rust/markdown.rs` 与 `rust/hook/render.rs` 里的围栏判断都只是同一个布尔翻转 (`starts_with("```") || starts_with("~~~")`)，不处理「闭合围栏不得短于开启」，也不处理 4 空格缩进代码块 —— 这些结构今天在 CLI 与 hook 两边都会被当散文修。前缀重放拿到的是「hook 永远不会与 linter 分叉，linter 哪天变强 hook 自动跟上」，不多不少；它承诺的是**等于当前 `limae --fix`**，不是「不破坏 CommonMark 所认为的代码」，后者是共享解析层与规范、黄金集的事，不能靠 hook 方案宣称已经获得。

真正的裁决依据是实测。orchestra 2026-09-11 组织了两个独立裁决方 (gpt-6-astra high、Claude Fable 5.1 high，footer 实读；两份报告由 orchestra 另开 PR 照原样入库到 `docs/research/`：[`hook-batch-fence-debate-astra.md`](../research/hook-batch-fence-debate-astra.md)、[`hook-batch-fence-debate-fable.md`](../research/hook-batch-fence-debate-fable.md) —— 注意 Astra 的结论钉在 head `b20bc03`，不是最终版)，各自从不同路径得到同一结论 —— 两份原案都不保证「逐批等于整篇」，推荐加了保守放行的前缀重放。Fable 用 52 条黄金 fixture 逐行切批实测：围栏计数方案 7 条与整篇分叉 (指令跨批 3、跨行反引号 span 4)，前缀重放 3 条，**且那 3 条是任何按批方案都消不掉的** (前一行已上屏、后一行才改变它的保护)；Astra 在 37 条可分批 fixture 上得到同形的 6 比 3，并给出一条条件性的不可能性结论：在「只能替换当前批、不能回改前批」的宿主合同下，任何算法都做不到全输入逐字节等于整篇，要么接受漏修歧义区段、要么改显示协议。本 ADR 取前者，就是下面第 3 步的两道 fail-open。

**每批是一个新进程，状态只能经磁盘传。** 状态就是**这条消息到目前为止的原文**，按批分文件：第 `k` 批把自己的 `delta` 原文写成 `parts/<消息实例键>/<k>.part`，实例键是 `message_id` 加 `turn_id` (为什么不只用 `message_id`，见下文「消息实例身份」) (沿用 `rust/hook/state.rs` 的 `keep`：`$TMPDIR/limae-hook/<session_id>/` 下、位置无配置项、不许落在任何 checkout 里、目录 `0700`、文件 `0600`、先写临时名再 rename)，然后：
...
6. **按原始 LF 字节截取本批**：在 `T'` 里跳过 `P` 含有的 LF 数，余下全部字节原样作为这一批的 `displayContent`，含 CR 与尾 LF，结尾与 `delta` 逐字节一致；空 `delta` 直接无输出；与 `delta` 相同则无输出。不用「最后 `lines(delta)` 行」这种说法 —— `"甲\n乙\n".split('\n')` 带一个虚拟尾空串，两种数法会差一行 (Astra 报告指出的测量陷阱)。验收要走 `Pipeline::fix`，不能拿 CLI 文件 I/O 的输出冒充 hook 的字节合同：CLI 那层会把 CRLF 归一化成 LF。
...
**清理**：消息的分片目录**不再由末批删除**，改由既有的 `prune` 按 `ORPHAN_RETENTION` (1 小时) 扫掉 —— 末批与它前面的某一批可能同时在跑，末批删目录会让那一批读到「目录不存在」、白等到期。会话目录仍按 `RETENTION` (24 小时) 清。**消息实例身份**：`message_id` 若被宿主复用 (中断后重发)，旧一代的分片会被新一代当前缀。Astra 用合成实验证明这不是理论风险的形状：当前代 index 0 已覆盖、index 1 未到而目录里留着旧代的 index 1、index 2 已到 —— 编号连续性检查通过，缺的那批本应是开栏行，旧文件却是普通文本，新消息的代码被改成 `foo ()`。**编号齐全只证明有这些文件，不证明它们属于同一次消息。** 同一报告也指出 `state::identifier` 是路径清洗不是标识符 (折叠非常规字符、截到 64 字符，不同输入可能同路径；从源码确立，不代表宿主发生过碰撞)。处置取最小机制：分片目录的键从 `message_id` 改为 `message_id` 加 `turn_id` (payload 已有该字段)；同 index 重送相同内容幂等，**内容不同则这一实例作废** —— 写一个作废标记，本批与后续各批原样放行、记 `siblings` / `incomplete`，不悄悄覆盖。两条前提由实现任务验证、不当保证，**验不成的处置也在这里定死，不许静默退回**：① `turn_id` 各批共有且随重发而变 —— 验不成 (字段缺席、不共有、或不随重发变) 则实现任务停下、回到本 ADR 与用户裁决，**不得静默退回只按 `message_id` 分目录**；若只是「不随重发变」，机制等于没加，失败方向仍是旧前缀污染 —— 那是协议缺信息，不能写成「index 0 清空就解决」。② 宿主 id 是 UUID 格式 (是则 `identifier` 的折叠对它是恒等映射) —— 验不成则实例键改用完整身份的安全编码或摘要 (如对 `session_id` / `message_id` / `turn_id` 原串取哈希再当路径段)，**不得继续拿 `identifier` 的折叠加截断当唯一键**。

**成本是「总量大、单批有界」**：每批重修一次前缀，总量是 `sum(|P_k|)`，不是一次 13 ms。两个裁决方的读数 (2026-09-11 本机 release)：Astra 12.9 KB 切 120 批合计 4.47 s 串行工作量 (不等于用户等 4.47 s，宿主是否重叠执行未测)；Fable 200 KB 单次 151 ms，100 KB 切 598 批合计 28 s，**但每批有界**。宿主对单批的预算是 10 s，所以单批不是问题，问题是极长消息的累计工作量。实现要给**原始字节数、分片数、单次等待**各设上限，超过就停修这一实例 (后续各批原样放行、记 `siblings` / `incomplete`)，**不能截掉旧前缀继续修** —— 那会重新丢失围栏与指令状态。上限的数值由实现任务定。
...
3. **等不齐就不修**：缺一批的目录，到期后该批无输出，`diagnostics.jsonl` 多一行 `siblings` / `incomplete`；补齐后同一批有输出。
   **实例身份两臂** (Astra 的合成形状)：目录里留一份旧代的 index 1 (内容是普通文本)，新代 index 0 已到、index 2 到达 —— 按 `turn_id` 分目录后 index 2 读不到旧分片、按缺批处理；同一目录下同 index 重送不同内容 → 本批与之后各批无输出、各记 `siblings` / `incomplete`，重送相同内容 → 幂等、照常修。
   **资源上限两臂** (§二「成本」的三项上限各测)：正臂是刚好不超过上限的消息照常修、诊断不多一行；超限臂是超过上限的那一批与之后各批都无输出、各记 `siblings` / `incomplete`，且此后的批**不得**被截掉旧前缀后照常修 —— 对照方法是在超限之后放一批围栏内的代码，它必须原样上屏。
...
实现 PR #180 (head `6913b06`) 与 orchestra 同日的中断实测带来五条落在上文之上的内容。正文各节不就地改写，以本节为准。

**一、三条实现期澄清。** 都不是对本 ADR 的偏离，是上文没写到、而实现必须有一个答案的行为契约：

- **末批不做 `unclosed` 判断**。§二 第 3 步第二道的条件是「还可能被**未来的行**闭合」，而末批没有未来的行。按字面读会以为每一批都判；照那个读法实现，每一条以未配对反引号结尾的回复都会永远少修一处。
- **行内指令点名不存在的规则 id 时，`fix` 拒绝，归到 `config`，不是 `crashed`**。这种输入来自回复正文里的 `inline-disable-*`，§五 的五个 Kind 表面上数不出它。归 `config` 的理由与该表保留 `config` 的理由同源：那是写的人自己的笔误，报成 `crashed` 等于让他去「报 bug」那个错误的方向找。`kind` 表有 `tools/check_repo_contracts.sh` 的承诺 2 盯着，所以这一种要在手册那一行写明两个来源。
- **payload 缺 `turn_id` 时无输出**。它是 §二 消息实例键的一半，缺了就判不了分片属于哪一次消息 —— 符合本 ADR「拿不准倒向不修」的方向。

**二、本 ADR 引的所有分叉计数，都是某一种切法下的读数，不是方案的固有属性。** §二 引了 Fable 在逐行切批下的「前缀重放 3 条分叉」，#180 用自己的切法 sweep 了全部黄金集 (52 条 fixture × 3 种切法)，读到的是**触发 `unclosed` 10 条、拼回后与整篇不等 6 条**。两者不矛盾，差的是怎么切批：一条 fixture 在一种切法下把开反引号与闭合放进同一批就不触发，换一种切法就触发。

耐久的是**形状**，不是数字：#180 那 6 条全是同一个形状 —— **被拒的中间批里既有未配对反引号、又有 span 之外可修的正文**。数字换个切法就变，形状不变。`spec/fixtures/span-across-line-break` 被拒但不分叉，因为被拒的那一批整批都在 span 里，正是同一个形状的反面。

**这条要当通则读，不只管这一处。** 同一个坑这一轮栽了三次：orchestra 那条「逐行与整篇的唯一分歧来自围栏」的读数被 Fable 指出是过度推广 (§背景「读数 B」末尾那句限定就是那次指出的结果)；Astra 主动把自己的结论钉在 head `b20bc03` 而不是最终版 (它自己防住了)；#180 的 sweep 是第三次。所以：**引用任何一个分叉计数时，同时写清它是在哪一种切法、哪一个 head 上读到的；不要把它当成方案的绝对值。**

**三、§二「成本」那三个上限 (256 KiB 原始字节 / 1000 分片 / 单次等待 2 s) 是拍的，不是量出来的。** #180 没有做任何新的计时测量：256 KiB 从 Fable 的「200 KB 单次 151 ms」线性外推取整到 2 的幂，1000 批参照 Astra 的 120 批与 Fable 的 598 批都远在其内，2 s 是原样沿用既有的 `SIBLING_WAIT`。三个数只说明「显然够用且有界」，**没有一个是从失效点量回来的**。后人要动它们，改的是 `parts::Limits::DEFAULT` 三个常量，不必以为是在推翻什么 —— 此刻它们背后一个测量都没有。

**四、中断的实测 (orchestra 2026-09-11 本机，隔离 lab 加采集 hook，esc 与 Ctrl-C 各一次)。** 读数是：**被打断的消息永远收不到 `final=true` 的那一批**。所以 §二 的清理一节说的孤儿分片**是常态、不是异常** —— 按年龄 prune 不是兜底，它是那条路径上唯一的清理者，必须真的承担这个职责。同一次实测也把 §二 列为「由实现任务验证」的前提 ① 验到了：`turn_id` 各批共有 (70 条真消息 0 例不一致)，且随重发而变 (两次中断后 `message_id` / `turn_id` / `prompt_id` 三个全换新)。**未覆盖的一臂照实写**：测的是「中断之后人再发一条新 prompt」，不是「同一条消息被宿主自己重投」(API 错误重试那一类)，后者没有样本。

**五、`message_id` 加 `turn_id` 之外的残余风险：用户 2026-09-11 裁决接受。** 用户的意思是「我其实还是不理解这个风险是什么，但至少从这个描述来看，它成立的概率很低。不要 over-engineering，该做做就好」(语音输入，经 orchestra 转述，措辞是意思不是逐字)。接受的依据是下面三件，三件要一起读：

1. **失效需要三个条件同时成立**：① 宿主对同一条消息复用 `message_id` 加 `turn_id`；② 两代的内容不同；③ 第 `k` 批读前缀的时候，某个内容不同的第 `j<k` 批还没落盘。第 ③ 条的措辞取自 #180 的实现方，比「index 0 输掉竞争」准 —— 作废是由**第一个到达且内容不同的新一代批次**触发的，通常是 index 0，但不限于它。
2. **爆炸半径有限**：hook 的 `displayContent` **只改屏幕、不落盘** (§一 引的 ADR-0009 §一 依据)。证据是 orchestra 2026-09-11 的实测：那 677 条回复的原文取自 transcript，而采集时 hook 正开着、其中 52% 需要修 —— 说明 transcript 里存的是未经 hook 改动的原文。所以最坏后果是「某一批围栏内的代码在屏幕上多一个空格，且不可撤回」，不污染 transcript、不污染任何文件、不进模型上下文。
3. **证据等级要原样保留**。orchestra 查过 Claude Code 二进制 (218 MB；grep 有分辨力，对照臂 `hookSpecificOutput` 命中 40、`displayContent` 命中 11)，**能确立**的是：客户端区分 client 侧与 API 侧两个 message id 空间，重试存在且被单独记账。**不能确立**的是：重投时 client 侧的 `message_id` 换不换。**字段名不是语义** —— 这个「能 / 不能」的划分不要压缩成「大概率会换」。

**这条不是「已排除」，也不是「已验证安全」。** 它是一个被明确接受的**已知残余风险**，接受的理由是概率低加爆炸半径有限，不是因为它被证伪。将来若真观测到一次，回到这一节，不要当新发现。
