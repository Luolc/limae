...
- **T1 收回「秘密 → 密钥」需要更细的判据**：v0.7.0 因同行锚点分不清「类别词的秘密」与「该译成密钥的 secret」而未收 (ADR-0007 §五)；用户立场是这类命中里 `secret` 指 access secret / token 时「零秘密」也该写「零密钥」。第二批若收回，判据要比同行锚点更细 (锚点限定具体凭证名、或要求同句)。
