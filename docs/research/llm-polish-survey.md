...
> 来源：Grok research agent 调研，2026-08-31；distill 决策另行进 spec / ADR，本文只记录证据。产品身份、热度、Deng spec 的 philosophy 清单已见 `docs/research/claudish-and-ai-slop-survey.md` §1，本文不重复考证，只补源码与机制。
...
下文路径均相对各自仓根。Gvozdev 仓没有测试套件 (`CLAUDE.md:7`)；Deng 仓对词典有 `dictionary/validate.py`，翻译器本身没有测试。

---

## 0. 先说结论 (判断)

两个项目都是 **LLM 整段改写**，不是 linter，也不是逐句 / unified diff 补丁器。对 `lo-md-lint` 可以借鉴的是合同，不是产品形态：

- Gvozdev：显示层或旁路文件、fail-open、prompt 很短、**代码围栏几乎只靠 prompt**，YAML frontmatter 是唯一真正受机械保护的结构。
- Deng：两份可复制 spec + 本地 PAW function；词典是给人读的 field guide，**运行时不进 prompt**；明确禁止机械换词。

中文不能直接套 Deng 的英文 Claudish spec。中文要处理的是翻译腔、量词、全角标点与术语对照；其中只有标点 / 空格 / 带锚点的选词已经由 `zh-typography` 一族与 zh-word-1 处理，其余才属于润色。

**与 `zh-typography` 一族 的顺序：先润色，后 lint。** 先 `--fix` 再润色，模型会把全角标点与 CJK 空格改回去；先润色再 `--fix`，排版家规可以修正模型的标点问题，又几乎不影响语感。人审的对象应是「润色 + 排版 fix」后的旁路文件，不是模型原始输出。

原型建议：做成 **可选的 agent skill (主路径) + 独立批跑入口 (次路径)**，默认产物为旁路文件；不要做成 `lo-md-lint` 的 CI 子命令，也不要把 LLM 接进 `--fix`。结构不变量用本仓已有的全局豁免解析器做确定性检查，黄金集测保护器，不测模型散文。
...
配置优先级全仓一致：flag 文件 > 环境变量 > settings 文件 > 内置默认 (`CLAUDE.md:25`)。flag 文件存在，是因为 Claude Code 的 `env` 在会话启动时冻结，无法中途修改 (`rewrite.sh:85`)。
...
| `MessageDisplay` | `rewrite.sh` | 60 s | 默认开启：显示层改写助手回复 |
| `PostToolUse` (matcher `Write\|Edit`) | `rewrite-md.sh` | 180 s | 默认关闭：要设 `CLAUDISH_MD_DIR` 才改磁盘 (`rewrite-md.sh:10`) |

斜杠命令 `/claudish` (`commands/claudish.md`) 不直接调模型。它只运行 `claudish-ctl.sh`，写 `~/.claude/claudish-{off,mode,style,lang,model}` 这组 flag，并在 `last` 时从 transcript 重打原文 (`claudish-ctl.sh:17`、`claudish-ctl.sh:210`)。

**没有独立 CLI 翻译器。** 离开 Claude Code 后，这组 hook 不运行。

`MessageDisplay` 会为每个流式 chunk 启动一个进程 (`rewrite.sh:5`)。hook 把每个 `.delta` 按 `message_id` 写到 `$TMPDIR/claudish-to-english/<session>/<message>/<index>.part` (`rewrite.sh:189`)，只在 `.final == true` 时拼出全文并调用一次模型 (`rewrite.sh:218`)。非 final 的 chunk：`append` 模式原样放行，`replace` 模式发送空字符串压掉原文 (`rewrite.sh:205`)。

### 1.2 Prompt / spec 怎么组织

没有 Deng 那种独立 spec 文件。System prompt **内联在 bash 里**，约一段话，可以由整份文件替换，不能合并。
...
分层如下 (后写的覆盖先写的，除非注释标明例外)：

1. **内置默认**，或由 style preset 整段替换默认：`tldr` / `5y` / `caveman` (`rewrite.sh:291`)。`tldr` 要求明显更短 (目标一半)，并且 **Omit fenced code blocks** —— 与默认的「围栏不动」相反 (`rewrite.sh:292`)。
2. **语言行** (有配置才加) 写在 prompt-file 检查之前：自定义 prompt 被视为完整 prompt，自带语言。
3. **`CLAUDISH_PROMPT_FILE` 整份替换**，读不到或为空则回退默认 (`rewrite.sh:304`)。自定义 prompt **优先于** style。
4. **人称锚定** 故意放在 prompt-file **之后**：输入是助手对用户的一回合，`I/me/my` 是助手、`you/your` 是用户；这是输入事实，不是风格，自定义 prompt 不能把它写错 (`rewrite.sh:314`)。
5. **用户原问** 从 `.transcript_path` 取最后一条非 meta 的 user 文本，截到 800 个 codepoint，只作上下文，禁止改写 / 回答 / 复述该问题 (`rewrite.sh:322`)。

Markdown hook 另有一份内联 prompt (`rewrite-md.sh:198`)，可由 `CLAUDISH_MD_PROMPT_FILE` 整份替换。它比显示 hook 多要求保留 Markdown 结构：
...
显示 hook 的默认 prompt **没有**提到行内代码、链接、表格。style preset 里的 `5y` / `caveman` 补了 “Keep technical terms, commands, and identifiers in their original form” (`rewrite.sh:293`)，默认 prompt 没有这句。

语言解析在 `lang.sh`：`CLAUDISH_LANG_FILE` > 显式 (含空) `CLAUDISH_LANG` > 项目 / 用户 `.claude/settings*.json` 的 `language` 键，顺序与 Claude Code 自己拼 `# Language` 相同 (`lang.sh:10`)。值会清洗：控制字符折成空格、最多 3 个词 / 30 个 codepoint (`lang.sh:24`)，因为这份 settings 会随仓库走，不可信。

### 1.3 词典

**没有词典。** 仓内没有 `dictionary/`、词表文件或注入 prompt 的术语对照。去 Claudish 的方式就是「更短、更日常」。Philosophy 清单在 Deng 仓，不在这里 (既有调研 §1.5)。
...
| Markdown hook | 整份 `.md` 的 body (frontmatter 已剥掉) | `sibling`：`NAME.plain.md`；`overwrite`：原地加 marker (`rewrite-md.sh:244`) | 是，经 shell `mv`，不走 Claude 的 Write，所以不会重入 PostToolUse (`rewrite-md.sh:19`) |
...
显示模式：`append` (默认) 在原文流完后附一块带标签的改写；`replace` 只显示改写，失败时重新打出全文原文 (`rewrite.sh:392`)。

`tldr` 允许改写比原文短很多，并且丢掉围栏；这不属于「结构保真」那条产品线。

### 1.5 怎么防改坏代码块 / 链接 / 术语

**机械保护只有 YAML frontmatter。** `rewrite-md.sh:146` 在调用模型前，若首行是 `---` 且后面有闭合 `---`，会把整块 (含定界行) 拆下来，只把 body 送给模型，写回时再拼回去。overwrite 的 marker 写在 frontmatter 后面，以免它不再是 frontmatter (`rewrite-md.sh:246`)。

围栏代码块：

- **长度检查**用 awk 按行首 ` ``` ` 翻转开关，把围栏行从「散文长度」中扣掉 (`rewrite.sh:223`)。这只影响「要不要改写」，**不**会把围栏从送给模型的文本中移除。
- **内容保护只靠 prompt** (“Leave fenced code blocks unchanged”)。没有占位符、没有二次 diff、没有「围栏字节级相等」的断言。
- 行首 `~~~` 的围栏：这段 awk **认不出来** (只看 ` ``` `)。本仓全局豁免支持两种定界 (`spec/rules.md`「全局豁免」第 1 条)，Gvozdev 的长度检查与它不一致。
- `tldr` 明确要求模型 **省略**围栏 (`rewrite.sh:292`)。

行内代码、链接 destination、表格分隔行、标题标记：Markdown prompt 用自然语言要求保留结构与 `link targets` (`rewrite-md.sh:205`)；显示 hook 的默认 prompt 连这些都没有提到。没有解析器。

术语：prompt 要求保留 fact / name / number / file path；没有词表，也没有「密钥 vs 秘密」这类对照。
...
**没有 accept / reject 检查。** 改写会自动出现。

人可以做的：

- `append` 模式下原文与改写同时显示在屏上，人可以只看原文。
- `/claudish last` 从 transcript 重打原文；回复以 `<!-- claudish:original -->` 开头，显示 hook 见到就跳过 (`rewrite.sh:197`、`commands/claudish.md:14`)。
- Markdown `sibling` (默认) 不碰原文，人可以查看 `.plain.md`。`overwrite` 没有预览、没有确认，只靠 marker 防二次改写 (`rewrite-md.sh:166`)。README 自己说弱模型会把真文档改坏 (`README.md:410`)。
- `/claudish off` 或 `touch ~/.claude/claudish-off` 暂停后续改写，已经写出的 `.plain.md` / overwrite 不会回滚。

人在流程中的位置是 **事后对照**，不是写盘前的关卡。

### 1.7 模型调用与成本

自己发请求，**不**复用当前 Claude 会话的补全 (oauth 模式是唯一例外，见下)。`providers.sh:136` 的 `llm_complete SYSTEM USER` 对四种 provider 各发起一次非流式补全：
...
| `codex` | CLI 自己的默认 | 无 (用 CLI 登录) | `codex exec --sandbox read-only`，在 `$TMPDIR` 下运行，不进仓库 (`providers.sh:285`) |
...
- 短于 200 个散文字符时跳过。
- 推理关闭 (`think: false` / `reasoning_effort: none`)，避免为一次改写消耗 reasoning token (`README.md:615`)。
- 打到输出上限的半成品会丢弃。
- 云端 provider 会把每条助手消息 (以及 Markdown hook 开启时的文件正文) 发送到外部 API；README 把选择云端 provider 本身视为 consent switch (`README.md:462`)。

`CLAUDISH_ANTHROPIC_AUTH=oauth` 从 macOS Keychain 或 `~/.claude/.credentials.json` 读 Claude Code 的 access token，改写记到用户订阅上。脚本自称 UNOFFICIAL，每次会话第一次成功改写都会警告 (`providers.sh:56`、`providers.sh:422`)。oauth 模式拒绝非默认 `CLAUDISH_ANTHROPIC_URL`，以免 token 发到代理 (`providers.sh:148`)。
...
两个 ProgramAsWeights function id 写死在 `translate.py:10`：`to-claudish = ca9d5165b6c8e6615529`，`to-english = e469f61ccab2699fbd51`。`paw.function(...)` 之后 `print(translate(sys.argv[2]))` (`translate.py:21`)。没有文件输入、没有目录扫描、没有 Markdown 感知。长文档需要自己把正文传进 argv。

README 说程序下载一次后在本地运行，不要传 `max_tokens`，PAW 在 EOS 自然停止 (`README.md:24`)。Live demo 在网站；本仓源码不包含网站。

### 2.2 Prompt / spec 怎么组织

两份 Markdown spec，供人复制进自己的 PAW program 或其他模型 (`README.md:38`)：
...
2. 改写必须是 paraphrase，不是对输入的回答；不添加事实 (`specs/claudish-to-english.md:5`)。
3. Prefer semantic compression：多句可以压成一句；不要一对一保留句数 (`specs/claudish-to-english.md:13`)。
...
6. 合法术语要保留 (`specs/claudish-to-english.md:176`)。
...
反向 spec 要求明显的风格变换 (至少改结构 / framing / 抽象层级等两项)，长度与输入大致相当，短输入通常仍是一句 (`specs/english-to-claudish.md:25`)。high-signal 词名单在 `specs/english-to-claudish.md:17`，既有调研 §1.4 已摘录。
...
- 每条恰好包含这些键：`slug`、`term`、`plain_english`、`explanation`、`category`、`aliases`、`example` (`example` 内恰好 `claudish` + `english`，且两者不得相同)。
...
- `specimens` 7 条，必须指向 `sources` 里的公开 URL，且注明是模型输出 (`dictionary/README.md:25`)。commit 上有 5 个 source，都是 GitHub 上的公开 issue 评论或 claudeisms 语料。
...
网站 vendoring 一份 commit-pinned 的 `entries.json`，不会在访客浏览器中现拉 GitHub (`dictionary/README.md:34`)。

与本仓 `spec/wordlists/` 的差别：Deng 词典是 **释义 + 例句的 field guide**；本仓 zh-word-1 是 `wrong` / `right` / `anchors` 的可执行替换 (当前 3 条)，`zh-tell` 与 `en-tell` 两族是禁词 / 句式表。Deng 的 30 条 **不能**当 zh-word-1 使用 —— spec 自己禁止机械替换，且多数词 (如 `canonical`、`drift`) 在技术文档里是合法用语 (`specs/claudish-to-english.md:178`；本仓 en-tell-3 只收了 `load-bearing` 一条，理由写在 `spec/rules.md` 的 en-tell-3)。

### 2.4 输入输出粒度

一次调用、一个字符串、stdout 输出一行 (或一段) 改写。粒度由调用方决定：README 示例是单句 (`README.md:30`)。没有 chunk 缓冲，没有「按 Markdown 块切」，没有旁路文件。正向 spec 允许输出比输入短很多 (`specs/claudish-to-english.md:201`)，因此 **不保证行数不变** —— 与本仓 `--fix`「不增删行」的契约 (`spec/rules.md`「处理单位」) 直接相反。
...
**没有。** CLI 打印结果，人自己决定是否贴回去。网站 demo 的人审 UI 不在本仓源码里，**未核实**。

### 2.7 模型调用与成本

`import programasweights as paw`，安装走 `--extra-index-url https://pypi.programasweights.com/simple/` (`README.md:11`)。权重下载后在本地运行。本仓源码看不到底层模型名、量化、硬件需求、是否走网络推理。

**美元成本、延迟、token 数均未核实** (本次只读源码，未跑 PAW、未装该 index)。与 Gvozdev 的差别是：这里没有「每个 Claude 消息打一次 API」的热路径，而是人显式调用翻译函数。
...
Gvozdev 的默认 prompt 要求 “Write the rewrite in the same language as the message you are rewriting” (`rewrite.sh:287`)。`lang.sh:28` 把 `简体中文` 作为合法语言名的例子。README 的 caveat (`README.md:305`)：
...
既有调研 §2.2 列出的中文特有层 (翻译腔、直译术语、公文黑话、接住体) 本次不重复考证；它们仍构成「Deng spec 覆盖不了的那一截」。

### 4.2 中文润色要多做的事 (判断)

直接拿 Deng 正向 spec 当中文 prompt，会漏掉这些：

1. **术语中英对照。** `secret` → 「密钥」而不是「秘密」，`cache` → 「缓存」而不是「快取」，是 ADR-0005 §四写出的动机。zh-word-1 已经能确定性修 3 组；润色 prompt 应要求「英文标识符保持原词，中文只用本仓 `zh-word` 一族的人话」，不要让模型另造「门控 / 一等公民」。没有唯一替换的词 (zh-word-2 的「秘密」) 只许标出，不许模型自行从三个候选中选一个写死 —— 那是人审。
2. **全角标点与中英空格。** 英文模型 (以及 Gvozdev 默认的 Gemma) 会输出 CJK 旁的半角逗号、括号贴汉字、漏掉 zh-typography-4 空格。Gvozdev / Deng 的 prompt 都不提 GB/T 标点。这不是语义问题，不该靠模型记家规。
3. **量词与语序。** 「正在飞」、翻译腔的 `He gave me a smile` → 「他给了我一个微笑」，是既有调研 §2.2 / §2.3 的中文层。Deng 的「降抽象、拆 not X but Y」对这类问题几乎无用；中文润色 spec 要写的是「删翻译腔、补量词、恢复中文语序」，并配合成例句 (ACME / Foo)，不要配真实业务句。
4. **中英两套指纹不要混进一张禁词表。** 既有调研 §2.4 与 ADR-0006 §五已经定了：共享句式 (否定平行、三段式)，词表各管各的。润色 prompt 也应分两段：英文 Claudish 用 Deng 正向 spec；中文用一套更短的中文条款 (套话、黑话、翻译腔)，不要把 `load-bearing` 的释义翻译成中文禁词。
...
- `zh-typography` 一族 的 `--fix` 改的是标点宽度与空格，**几乎不改选词和句式**，所以不会破坏 Deng 意义上的语感 (哪几个命题、怎么压缩)。这是家规落地，不是「把人话改回腔」。
- zh-word-1 的 `--fix` **会改词**。若润色故意在无锚点行里写了「代币」(钱的意思)，zh-word-1 因无锚点不动；若同行出现了 `token`，zh-word-1 会改成「令牌」。这是 zh-word-1 的既有契约，不是润色引入的新风险；experimental 默认关闭，原型可以先只跑 `zh-typography` 一族。
- `zh-tell` 与 `en-tell` 两族全是 non-fixable，`--fix` 碰不到套话。套话要么被润色删掉，要么留下作为 warning。
- Deng 允许减句、Gvozdev 的 `tldr` 允许减半并丢掉围栏。`--fix` 不增删行 (`spec/rules.md`「处理单位」)，所以 **减句是润色自己的事，lint 既修不回来，也不该修**。结构不变量检查要拦的是围栏 / 链接被改坏，不是句数。
- 若人审的是模型原始输出、写回后再 `--fix`，人看到的和入库的不一致。所以人审应发生在 `--fix` 之后。
...
tracker 已有的「LLM 润色原型：Deng spec 当 prompt、旁路文件、不进 CI」(`docs/tracker.md:28`) 与这条同向，可以在 ADR 中收成合同，不必另起产品名。

### 5.2 模型调用

**建议默认走「当前 agent 会话」**，不要在 linter 进程里再打一次 API。理由：
...
若做独立批跑入口，provider 合同宜借鉴 Gvozdev 的 **fail-open + 默认本机**：

- 默认不打云。本机 ollama 或用户显式配置的兼容端点。
- 半成品 (打到 `max_tokens` / `length`) 丢弃，不写旁路文件。
- 关掉 reasoning (`think: false` / `reasoning_effort: none`)；润色不是解题。
- **不要**借鉴 `CLAUDISH_ANTHROPIC_AUTH=oauth`。

Prompt 组织建议分层，借鉴两侧各自擅长的部分，不要把一份 200 行英文 spec 直接喂给中文：
...
3. **英文 Claudish 段**：Deng 正向 spec 可以整份作为附录，只在输入含英文行文时启用；不要把 30 条词典贴进 prompt (spec 禁止机械换词，而且会教模型「看到 `canonical` 就删」)。
4. **人称 / 文类锚定** (Gvozdev `rewrite.sh:320` 那一层)：文档不是助手对用户说话，「我」不是模型。
5. (可选) 本次 `check` 的 tell / word 两类家族的 findings，作为 hint 而不是必改清单。
...
借鉴 Gvozdev 的 `CLAUDISH_STUB=1` (`rewrite.sh:279`)：保护器先把不变量拆成占位，再跑一个 **确定性的破坏桩** (例如把所有可见散文换成 `X`，或整段大写)，再嵌回。黄金断言是「桩跑完，不变量区域仍与 `.in` 字节相同」。这样测的是解析 / 占位 / 嵌回，不测 Gemma 会不会听话。

第二道检查发生在真模型路径上，但仍是确定性的：润色产出旁路后，用同一套豁免解析器从 `.in` 与旁路抽取不变量，不相等就 fail-open。这道检查可以留在 skill / 独立入口里，**不必**进 `lo-md-lint` 的 required check。

**不要**让模型「自己保证围栏不动」而不做机械剥离。Gvozdev 只机械保护了 frontmatter，围栏仍送给模型；`tldr` 还会要求它删围栏。本仓已经有豁免解析，保护器应 **先拆后嵌**，prompt 里的 “Leave fenced code blocks unchanged” 只作辅助，不是唯一保护。

现有 runner 的三条断言 (`spec/README.md`「runner 的判定」) 仍然只服务 lint。润色保护器是第四类测试，断言形状不同 (区域字节相等，而不是行号 + 规则 id)。可以仍放 `spec/fixtures/` 用新后缀，也可以放在实现自己的测试中；ADR 要定的是「不变量清单与 fail-open」，不是目录。

### 5.5 与 heuristic learning 的接口

ADR-0005 §六要的是：pair 只 **提议**规则，人审后进 `spec/`，新规则必须跑全量黄金集。既有调研 §4.4 已说明这不是现成产品，而是运维模型。润色原型要预留的不是训练循环，而是一份 **可蒸馏的 pair 记录**：

每条被接受的润色留下：

- before：润色前的原文 (未经 `--fix` 预处理)
- after：人审接受的文本 (已经过结构检查与 `zh-typography` 一族的 `--fix`)
- 可选：模型原始输出 (人改之前)，用来看人改了什么
- 元数据：日期、模型名、用了哪一版 prompt、是否开过 experimental。**不要**写路径里的真实业务名；fixture / 语料只用 ACME / Foo (`AGENTS.md`「隐私边界」)

蒸馏方向 (只有能从 pair 中稳定归纳出的，才接 HL)：
...
接口形状：润色侧 **只产出 pair**；提议规则是另一条 agent 任务，读 pair、写词表补丁 + 新 fixture 草案，跑全量黄金集，回归失败就丢弃这条提议。不要在润色进程里改 `spec/wordlists/`。这与翁家翌 HL 的「策略可读、可回归、可删」同构，也与 ADR-0007「词表是规范、加一条词就是改规范」同构。
...
不做：CI required check、warning 计数触发、overwrite 默认开启、把 Deng 词典当 zh-word-1、在 linter 里内置 oauth / 云厂商 SDK、用 `.fixed` 锁散文。
