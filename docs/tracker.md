...
- **Rust 发版准备** (2026-09-06)：许可证尚未选定；认证等待 machine-setup 的开发机发布手册及用户创建 scoped token，首发后配置 GitHub Actions Trusted Publishing。当前不视为已取得可用凭据，也不阻塞代码迁移与无需认证的发布预演。实际发布仍在本轮授权停止点之外。
