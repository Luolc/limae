...
- **第 8 道门守的是 linter 自己，不是词典内容**：`spec/lexicon/zh.toml` 是干净的，所以第 7 道门在行号映射正确、错误、乃至整个删掉时都打印 `OK`。`tools/check_lexicon_lint.sh` 拿行号固定的 fixture 驱动这个工具，逐条断言报出来的 toml 行；它把真词典再跑一遍是**自己的对照臂** (其余各臂都断言「应该报」，一个什么都报的工具能全过)，不是第二道内容门。**别把这类测试写进 example 的 `#[cfg(test)]`**：`cargo test --locked` 只编译 example、不跑它们的测试 —— 2026-09-10 本机两臂实测，一个恒失败的 `#[test]` 在 `cargo test --locked` 下退出 0 且名字都不出现，只有 `--all-targets` 才退出 101。写在那里的回归测试是一条永远不会红的空洞。
