...
`pyproject.toml` 的版本是 **0.3.1**，最新 tag 是 `v0.3.1`。消费方引用的永远是 tag 而不是 `main` —— 不改版本的改动 (比如文档) 合入后 `main` 会领先于最新 tag，那不代表有新版本可升。五个 tag 的语义：
...
| `v0.3.1` | 含假名的引用 span 全局豁免 (逐字引用的日文原文不再被改写) | 是，no-op (不含此类引用的文档零行为变化) |
