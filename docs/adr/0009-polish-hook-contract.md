...
hook 调引擎的隐私约束与 ADR-0008 §三 + PR #37 落地的 `src/limae/engines.py` 一致，不额外放宽也不额外收紧：

- 引擎在一次性临时目录里跑，只拿白名单环境变量 (`PATH`、`HOME`、`TMPDIR`、locale、`TZ`，加该家自己的凭证与 base URL 变量)，不透传 `PWD`、`GIT_*` 全族、宿主自标变量，细节见 `docs/research/polish-engine-cli-behavior.md` §一。
- hook 送进引擎的**只有待润色的那一段文本** (拼好的整条助手消息)，不带仓库上下文、不带其它待处理内容。
