...
- `wordlists/` —— 词表：A1 / A3 / A4 / T1 的判定数据，格式与匹配语义的正本是 `rules.md`「词表」。

三份同目录、同 PR 改，不会漂移。改规则的顺序固定：先改这里，再改各实现。

## 词表

`wordlists/A1.txt`、`A3.txt`、`A4.txt` 一行一条 (`#` 注释、空行忽略)，`wordlists/T1.toml` 是 `wrong` / `right` / `anchors` 的 `entries` 数组。
