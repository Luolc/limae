...
调研日期：2026-09-09。只读，未改本仓，也未改上一份报告。本地对照仓 `~/3p/huacnlee/autocorrect` 仍停在 `e1a75da` (2026-06-23)，与 `origin/main` 相同；**源码结构没变，但各注册表上的版本已经分叉**，所以包名 / 版本 / 日期全部重新核对，不沿用 2026-08-31 那份 internals 调研的清单。

## 一句话结论

**不要学 AutoCorrect 做「Rust 核心 + 每家一套 FFI 绑定」。** 这条路实际带来的是版本分叉，不是覆盖面。

limae 短期只有 CLI (format / polish / lint)，明确不做 LSP。跨 Python / Node / Rust / 纯文档仓的统一策略是：

**以 GitHub Release 的预编译二进制为唯一来源，外面套两层薄启动器 —— PyPI 的 maturin `bindings = "bin"` wheel，和 npm 的 biome / dprint 式平台包。** 不要 napi-rs，不要 pyo3 扩展模块，不要 Ruby / Java / Wasm SDK。

### 对上一轮推荐的修订

上一轮「先做 PyPI wheel + tiny pre-commit mirror」**作为 Python 路径仍然成立**，但不再是「下一件唯一的事」。Node / TypeScript 被点名为同等重要之后，**只发 PyPI 会把前端仓排除在外**；反过来，只发 npm 会把 Python 仓排除在外。两条薄启动器共享同一份 Release 二进制，维护面几乎不翻倍。会翻倍的是 FFI 绑定，不是 CLI wrapper。

---

## 1. AutoCorrect 现在实际发了什么 (2026-09-09 逐个核实)

源码仓仍是「一个 workspace 里有 node / py / rb / java / wasm / lsp / cli」(`Cargo.toml` members，读到原文)。**注册表上的版本与 git tag `v2.16.3` 对不上。**
...
README 徽章仍同时挂着 Crate / NPM / PyPI / Gem / Maven (本地 README.md L9–13，`e1a75da`)。徽章让人觉得「全家桶同步」，注册表的数据不是这样。

**2026-08-31 那份 internals 调研中仍有效的部分：** Node / Python / Ruby / Java / Wasm 目录都还在，FFI 接到同一份 `Ignorer`，测试仍是各绑一层冒烟 (`autocorrect-node/__test__/index.spec.mjs` 五条 `Hello你好.`)。**失效的是「都包了」隐含的「都在发」** —— Java 停了 16 个月，Node / PyPI / crates 停在 2.14.x，只有 Release 二进制和 RubyGems 跟到 2.16.3。
...
- 装完后导出 JS API：`format` / `formatFor` / `lintFor` / `loadConfig` / `Ignorer` / `run` (`index.js` L255–262，napi-rs 生成)。
- `bin.autocorrect = ./cli.js`，里面就是 `require('./index.js').run(process.argv)` —— CLI 绑在同一份 `.node` 上，不是独立二进制。
- 平台覆盖靠 `optionalDependencies` 的五个分平台包：`win32-x64-msvc`、`darwin-x64`、`darwin-arm64`、`linux-x64-gnu`、`linux-x64-musl`。**没有 linux-arm64。** GitHub Release 倒是有 `linux-arm64` / `linux-musl-arm64`。绑定矩阵和 CLI 矩阵已经分叉。
- 安装时**不本地编译**：预编译 `.node` 放进分平台包。平台不匹配就 `Failed to load native binding`。
...
`wasm-pack` 打出来，无 `bin`，unpacked ~3.2 MB。README 定位是 Chrome 插件 / 编辑器内嵌。上周 76 次下载，几乎没人把它当 CLI 用。
...
这就是 Node 生态的 pre-commit 组合：husky / simple-git-hooks 的 hook 调 `lint-staged`，后者调 `node_modules/.bin/autocorrect`。
...
还有一个机制问题 (读到原文，不是推断「他们懒」)：`release-crate.yml` 只在 `workflow_dispatch` 上跑，发布步骤却要求 `github.ref` 是 tag —— 从 main 手动点 Dispatch 时 ref 是 `refs/heads/main`，publish **直接跳过**。这能解释 crates.io 停在 2.14.2。
...
**每次发版：** 渠道数 × (编、测、token、失败重跑)。AutoCorrect 只给 Release 二进制 / Node / Wasm 自动发布；Python / crate / Ruby / Java 改成手动。改成手动的那些渠道最先停更。
...
**下载量对照 (npm last-week，2026-08-31–09-06，api.npmjs.org)：** `@biomejs/biome` 1318 万，`oxlint` 1904 万，`dprint` 20 万，`autocorrect-node` 3948，`@huacnlee/autocorrect` 76。AutoCorrect 的 Node FFI 没换来生态级用量，却付了全套矩阵的代价。

**给 limae 的数字直觉 (推断，不是实测分钟数)：** 只发 CLI 二进制 + 两个薄启动器，发版是「一组 compile + 两段打包上传」。每加一个真正的 FFI 生态，大约再加一条 5+ 平台的编译矩阵和一套冒烟。AutoCorrect 加到 5 家绑定之后，有 3 家已经落后两个小版本以上。limae 现在没有专职发版团队，复制这条路会先在同步上失效。

---

## 5. Node / TypeScript：Rust CLI 怎么进前端仓

主流不是 FFI，而是 **「npm 包里放预编译 CLI」**。装完 `npx <tool>` 或 `package.json` 的 `scripts` 就能跑，**安装不编译**。
...
**dprint (`dprint` 0.57.4，2026-09-05)：** 同样 `bin` + 15 个 `@dprint/<os>-<arch>-<libc>` optionalDeps，另有 `postinstall: node ./install.cjs` 做下载兜底。配置是自己的 `dprint.json`，不放进 `package.json` 业务字段。

**oxlint (`oxlint` 1.82.0，2026-09-07)：** 看起来像 biome，实际是 **napi-rs** (`napi.packageName = @oxlint/binding`，19 个 binding 包)。它有理由：oxlint 要当 ESLint 的替换，编辑器和 JS 工具链需要 in-process。这是「JS linter 写在 Rust 里」，不是「给 Markdown 文件跑一个 CLI」。

**autocorrect-node：** 见上，napi + 顺手挂了 `bin`。前端仓实际用的是那个 `bin`，不是 `format()`。
...
3. **pre-commit 框架** 在 Node 仓里也能跑 (ruff-pre-commit 那套)，但前端仓通常没有 Python；不要把 Python 预提交框架当成 Node 的主入口。
...
limae 短期三个功能都是命令行；`polish` 还要再 exec 别人的 CLI。没有「在 Next.js 请求路径里 `import { format } from 'limae'`」这种需求。不做 LSP，编辑器插件要接也可以 spawn CLI (markdownlint-cli2、dprint 都这样)。

FFI 的唯一好处是少一次进程启动。对「扫一批 `.md`」这个负载，启动成本可以忽略。它换来的是 napi 矩阵、abi3 wheel、Node 版本政策，以及「绑定测过了但 CLI 行为没测」的假绿。
...
**一条能覆盖大部分场景的路：以 Release 二进制为源，按生态各做一份薄启动器，不做原生绑定。**
...
三家薄启动器 (Release / PyPI / npm) 编的是**同一份 binary**，不是三份语义。pre-commit 继续走 Python wheel，因为 pre-commit 自己是 Python —— 这不是「偏袒 Python」，而是 hook 框架的运行时。
...
| 上一轮 | 这一轮 |
| --- | --- |
| 下一件事 = PyPI maturin bin wheel | **改成：以 Release 二进制为源，PyPI 与 npm 两个薄启动器并列。** 若必须排期，可以先 PyPI (用户自己的 Python 仓先吃到)，但文档和发版设计一开始就要给 npm 留位，别把根 `pyproject.toml` 做成「Python 专属产品」，以至于 npm 要另起一套编译。 |
...
**硬说「只做 PyPI 就覆盖别人的仓」不成立。** 硬说「要服务 Node 就得 napi-rs」也不成立 —— biome 和 dprint 用 optionalDeps 装 CLI，才是 Node 生态给 Rust 工具的标准答案。
...
**代价：** npm 多 8 个左右的平台包和一段发布脚本，比 napi 矩阵小一个数量级。**不要**为了「完整」去发 Wasm / gem / jar。AutoCorrect 用五年证明了那些渠道会先停更。
...
- **推断：** 「Release 产物直接打进 npm 平台包、不必重编」在 biome / dprint 上成立，limae 没做所以没测；AutoCorrect crate workflow 的 tag-guard 导致停更，是读 YAML 得出的机制推论，没有翻他们的 Actions 日志。
