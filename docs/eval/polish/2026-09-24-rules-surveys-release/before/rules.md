...
- **同形但不是指令的三种文本**：围栏代码块里的同形文本 (它是代码)、不独占一行的 HTML 注释、名字拼错的注释 (如 `limae-disabled`) —— 都按普通文本处理，照旧参与各规则判定，不另报错。
...
整份文件不该被检查时用忽略文件 (ignore file) `.limae-ignore`，内容采用 gitignore 模式语法 (pattern format：`#` 注释、空行、`!` 否定、`/` 锚定、`**`)，模式相对该文件所在目录；字符类的路径段边界按下述兼容约定处理。
...
字符类 (character class) 按 Python 参考实现的兼容行为处理：`/` 先作为路径分隔符，字符类必须在单个路径段内闭合。`[ab].md` 匹配 `a.md` / `b.md`，`a/b.md` 按普通路径匹配；`[ab.md` 与 `[a/]b.md` 都不产生有效模式，整行不匹配任何路径。这项约定只处理未在路径段内闭合的字符类，不把其它解析错误改为静默跳过，也不宣称覆盖 Git 的全部边界行为。
...
- **判定与报告**：由两个消费型模式组成。标点前模式匹配「非空白且不是 `—` `⸺` `|` 的字符 + 一段连续的空格 (U+0020) + 集合 `，` `。` `、` `；` `：` `？` `！` 里的全角标点」；标点后模式匹配次序相反的「集合内标点 + 一段连续的空格 + 非空白且不是 `—` `⸺` `|` 的字符」。两个模式各自从行首向行尾独立扫描，每个模式内只取从左到右的非重叠匹配；每次匹配消费空格串及两端字符，所以下一个匹配不能复用同一模式已消费的端点。同一段字符仍可由两个独立模式各匹配一次，例如 `， ，` 报两处。合并两个模式的匹配时按匹配起点升序报告，起点相同时标点前模式在先。
- **修复**：报告时的消费只决定 finding 数，不缩小修复范围；每一段局部满足上述相邻条件且未被豁免的空格都删除。因此连续交替的标点全部收拢，即使其中某段空格没有形成单独的非重叠 finding。
- **另一端排除 (破折号豁免)**：`—` (U+2014) 与 `⸺` (U+2E3A) 直接排除在模式的另一端字符类之外，不是先匹配再过滤；zh-typography-8 刚补的破折号旁空格不能被本条回收。
- **另一端排除 (表格豁免)**：`|` 直接排除在模式的另一端字符类之外，不是先匹配再过滤；那是表格单元格边界的 padding，属于 Markdown 语法排版而不是行文。
...
， ， ，          → ，，，          (zh-typography-11 ×2；两个模式各一处)
， ， ， ，       → ，，，，        (zh-typography-11 ×4；两个模式各两处)
— ， ，           → — ，，          (zh-typography-11 ×2；排除端点不吃掉后一处匹配)
a | ， ，         → a | ，，        (zh-typography-11 ×2；排除端点不吃掉后一处匹配)
你好， —— 世界   → 不变 (破折号豁免)
| 单元格结尾。 |  → 不变 (表格豁免)
```
