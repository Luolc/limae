...
6. **配置**：新增用户级配置 `~/.config/limae/limae.toml`；仓级覆盖用户级；用户级里有一个开关，打开后 hook 根本不读仓级、只认用户级，默认关。**这个文件存在就读、不存在就走默认，任何时候都不替用户创建它** —— 绝大多数用户会一直用默认值，缺席是正常状态，不是待修复状态。

前四条与第六条的前半由 orchestra 在 brief 里转达；第五条与第六条的后半是用户在 orchestra 的会话里口头补的 (语音输入)，经 orchestra 转述、本 ADR 未亲闻，措辞是意思不是逐字。
...
### 三、配置解析链：用户级为底、仓级整份覆盖、一个开关关掉仓级

hook 读的规则配置是两层：

1. **用户级** `~/.config/limae/limae.toml` (`~` 取 `$HOME`，所有平台同一路径，不按平台换目录、不读 `XDG_CONFIG_HOME` —— 只有一条路径要写进手册；真有人需要再加)。文件不存在等于这一层为空。

2. **仓级**：从事件的 `cwd` 起按今天的 `find_config` 发现 —— 逐层向上，每层先 `limae.toml` 再 `pyproject.toml` 的 `[tool.limae]`，**碰到含 `.git` 的那一层就停**，不跳出仓库继续上行；`cwd` 不在任何仓库里就一路走到根，与今天一致。

**用户级文件只读、永不创建**，这是契约，它排除几种很自然但错的做法：`install.sh` 不 `mkdir -p ~/.config/limae`，首次运行不写一份带注释的模板，缺文件不报 warning、不记诊断。**「不存在」与「存在但读不了」必须是两种不同的行为**，否则用户改错一个键，行为悄悄退回默认、而他以为改生效了：

| 用户级文件 | hook 的行为 |
| --- | --- |
| 不存在 | 这一层为空，静默；仓级找到用仓级，否则默认集，**照修** |
| 存在、合法 | 按本节的链用 |
| 存在、不是合法 toml，或触犯 `spec/rules.md`「配置错误」清单，或 `[hook]` 的值不是布尔 | **这一批不修**、原文放行，记一行 `fix` / `config` (§五)；仓级在场也不用 —— 与 CLI「不得退化成默认集」同一口径 |

第三行不是「退回默认」而是「什么都不修」，这就是它与第一行的分辨力：屏幕上排版全部停修，用户能看出来；诊断行说的是哪一类。要单独验一份用户级文件，按手册那条「手工喂一条批次进去」的做法喂一批含已知违规的文本，看它修不修、`diagnostics.jsonl` 有没有 `config` —— 不为此加 CLI flag。

**覆盖是整份的，不逐键合并**：仓级找到了就只用仓级，没找到就用用户级，都没有就是默认集。这沿用 `spec/rules.md`「来源与优先级」既有的口径 —— 「一次运行只有一个来源生效，不做逐键合并」，CLI flag 与配置文件之间就是这样定的；两层之间再发明一套逐键合并 (`disable` 列表是并集还是替换、`severity` 表怎么叠) 只会多出一批要定义、要测、要解释的边角。

**开关**写在用户级文件的 `[hook]` 表里：

```toml
[hook]
ignore_repo_config = false   # 默认；true 时 hook 不读仓级，只认用户级
```

`[hook]` 表只在用户级文件里有意义，仓级文件里出现按配置错误处理 (与 `spec/rules.md`「配置错误」清单同款处理：这一次运行的配置不可用)。值不是布尔也是配置错误。

**这两层只有 hook 读。** `limae [--fix] FILE...` 的发现规则不变 (仓级、止于 `.git`)。理由是两者的读者不同：CLI 的输出要在这台机器、CI 与消费仓的 pre-commit 之间可复现，README「规则选择写进配置文件，仓库自己跑的 limae 与这条 hook 读到的才是同一份」说的就是这件事，一份只在某人家目录里的文件会让本地跑与 CI 跑分叉；hook 修的是这个人屏幕上的回复，谁在读就按谁的口味。开关存在的理由也在这里：仓级配置是给那个仓的文档写的 (本仓开着实验规则)，不一定适合聊天回复。
...
| Kind `config` (`Misconfigured`) | **留、换来源** (预判说删) | 旧来源是 `[polish]` 表；新来源是 §三 那两层规则配置读不了。把它并进 `crashed` 会把用户自己的 toml 笔误报成「这是 bug，请报」，让人去错的地方找 |
...
- **ADR-0009 §七 的分发计划不变**：稳定后经 `machine-setup` 进用户级配置。用户级 `~/.config/limae/limae.toml` 的分发同样走那条路，不由本仓直接写家目录。
- **不做的**：不改 `spec/`，规则语义一个字不动；不给 CLI 加用户级配置；不带围栏之外的跨批状态；不为 Codex 找替代事件。
...
5. **配置链三臂**：只有用户级 → 用用户级；用户级加仓级 → 只用仓级 (拿一条只在用户级关掉的规则做探针，仓级在场时它必须又报)；用户级 `ignore_repo_config = true` 加仓级 → 只用用户级。外加两臂分开「不存在」与「读不了」：用户级文件不存在 → 照修、`diagnostics.jsonl` 不多一行；用户级文件解析失败 → 无输出、记一行 `fix` / `config` (仓级文件里出现 `[hook]` 同上)。再加一臂守「永不创建」：`$HOME` 下没有 `.config/limae/` 时跑完一批，那个目录仍然不存在。
6. **CLI 不受影响**：`limae FILE` 在家目录有用户级文件时输出与没有时逐字节相同。
