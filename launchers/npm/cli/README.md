...
This package carries no logic of its own. `bin/limae.js` finds the `@limae/<platform>` package that npm installed for the current machine and executes the binary inside it. That binary is the same file that ships as `limae-<target>.tar.gz` in the GitHub Release of the same version; every platform's copy is verified against its SHA-256 checksum when the packages are built.
