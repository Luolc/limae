...
- **limae 自己的写回只写你点名 (或 `--all` 选出) 的文件**；引擎只回正文。写回之前 limae 核对目标仍是跑前那一版，不是就拒绝写、保留你的版本 (`not written`，退出码 1)。视图另有一道绊线：引擎要是在视图里写了东西，说明只读设置没有生效，这一轮不写、整个运行停下。**这是事后检测加冲突拒绝，不是写入隔离**：引擎本身与你家目录里的引擎配置 (实测 `~/.claude/settings.json` 的 hook 在文件模式下照跑) 仍可能在视图之外写盘，绊线看不见仓库之外、`.git` 内部与网络。
