...
**当前不提供用户侧的词表增删接口**：词表写死在规范里、随包分发，既没有配置键，也不读任何外部词表文件 —— 用户改不了词表，只能用 `disable` 整条关掉规则、或用行内指令就地豁免。一开始不设计复杂形态；将来真要开放 (项目专用词表、`extend_wordlist` 这类键) 另起 ADR，那时才谈合并语义与校验。
...
**这五条与它们的词表条目是可调项，不是本 ADR 定死的东西。** 具体收哪几条规则、每张词表收哪些词，由实现方按证据挑定，用户没有逐条审；本 ADR 授权后续随时增删 —— 加词、删词、加一条 A5、把某条降回去，都不需要新的 ADR。**校准与验收不阻塞合入**：experimental 的定位本来就是「边开发边校正」，误报要靠真实语料一轮轮打磨出来，而不是靠合入前审完一张清单。真正需要另起 ADR 的是形制变化 (前缀约定、词表位置与格式、逐行 / 文档级模型、默认严重度这些)，不是清单内容。
...
**校准中删掉的一条：`秘密` → `密钥`。这是本批的取舍，不是定论 —— 用户不认同删除的理由，但不阻塞本批合入。** 删之前它在四份语料上命中 56 处 (`machine-setup` 43、本仓 9、`wealth-management` 3、`butler` 1)。

- **用户立场**：如果一份文档里讲的 `secret` 就是 access secret / token，那么「零秘密」同样是错的，本该写「零密钥」 —— 这些命中里有相当一部分并不是误报，而是真的该改。
- **本批仍不收的理由**：`machine-setup` 与跨仓守则里的「秘密」多数是**有意当类别词 (category term) 用的** —— 密码、key、token、助记词统称秘密，守则原文就是「……等任何秘密」。T1 现有的判据只有「同一行出现锚点」，它分不清「作类别词的秘密」与「该译成密钥的 `secret`」；而 T1 是 fixable，判错就会真的改坏文本。宁可这一批漏报，不要在可自动修复的规则上误报。
- **第二批要收回这条，需要比同行锚点更细的判据**：例如把锚点限定为具体的凭证名 (`OP_SERVICE_ACCOUNT_TOKEN`、`api_key` 这类) 而不是 `secret` / `key` 这种泛称，或者把「同一行」收紧成「同一句」。判据长出来之前不收。

调研 §5.3 早就说过 `secret` 要分语境；这一轮 dogfood 把话说得更具体：**同一行的英文锚点还不够细，不足以当那个语境。** 删除的事实与理由记在 `T1.toml` 的注释里，backlog 由 orchestra 在合入后记进 `docs/tracker.md`。
