...
- **对标 ruff 之于 Python**：主实现是 Rust —— 一个 Rust 写的 Markdown lint，可被 Python / Node 生态经 pre-commit、包管理器等集成，也能直接当命令行工具用。Python 那份留作参考实现 (reference implementation)，命令已改名 `limae-python` 并标记 deprecated (ADR-0015)。
...
默认只检查、不修复；要自动修复就自己加 `args: ["--fix"]`。`id: limae` 装的是 Rust 实现：pre-commit 按它的 [Rust language 合同](https://pre-commit.com/#rust) 用 Cargo 把 binary 编进自己的缓存，消费方不必预装 `limae`，但机器上要有 Rust 工具链，且第一次安装要联网、要等一次编译 (之后走缓存)。换成 `id: limae-python` 就退回 Python 参考实现 (见「Python 参考实现」一节)。
...
```sh
uv sync                           # 建 .venv、装 dev 依赖 (参考实现与质量门要用)
...
cargo test                        # Rust 测试
uv run pytest -q                  # Python 参考实现的测试
```
...
Python 那份实现**已 deprecated**，但不会删：它是差分测试的对照一侧，每次质量门都跑，规则语义有分歧时以 `spec/` 为准、以它为参照 (ADR-0015)。

改动只有名字：命令从 `limae` 改成 `limae-python`，pre-commit hook id 从 `limae` 改成 `limae-python`。**行为、输出与退出码一个字没动** —— 弃用不经 stdout 上的横幅宣告，那条流是 hook 与 pre-commit 读的协议，往里写字会把它弄坏。
...
```sh
uvx --from git+https://github.com/Luolc/limae@<tag> limae-python --all
```

已经在用旧写法的消费方不会被这次改名打红：`rev` / `@<tag>` 钉在改名之前的版本上时，装出来的仍是那时的 `limae`。要退回参考实现，或者把 id 换成 `limae-python`，或者把 `rev` 钉回旧 tag。
