...
回复下方那一块**只列改动的行**，每处写成 `原` / `改` 一对；整段一字未动时写「── 润色 ── 无改动」。不重贴整篇的理由是它没有信息：那正是读者刚读完的文字，而模型每次只移动其中百分之几的字符 (本仓 2026-09-02 的记录：sonnet 平均 2.9%、opus 2.0%)，混在一屏散文里看不见。列出改动之后，「有没有生效」与「改得对不对」都是一眼的事。
