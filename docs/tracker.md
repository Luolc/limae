...
- **Rust 全量迁移**：[ADR-0015](adr/0015-rust-migration.md) 的阶段 A 已完成 (2026-09-06，验收见 [PR #105](https://github.com/Luolc/limae/pull/105))：临时 `limae-rs` 已实现全部确定性规则、Markdown 保护、配置、行内指令、文件选择与写回；共享黄金集和 Python / Rust 真实进程差分通过。配置错误按 [PR #75 的协调决定](https://github.com/Luolc/limae/pull/75#issuecomment-5558344043) 比较退出码、类别、来源路径、键与既有手写消息子串，保留诊断而不要求回显未校验值。完整质量门见 `AGENTS.md`。B1 已在 [PR #108](https://github.com/Luolc/limae/pull/108) 完成 Cargo 源码包解包测试与安装、离开源码后的独立运行，以及 Linux GNU / musl 业务验收；分发与安装验收见 [分发说明](knowledge/rust-binary-distribution.md)。B2 已在 [PR #110](https://github.com/Luolc/limae/pull/110) 完成独立 `limae-rs` pre-commit opt-in 与原 Python 入口的隔离安装、检查及修复验收；阶段 B 完成，十二条门已随 #111 同步。下一项 C1 迁移润色子进程的受控执行与回收；依赖与验收合同见 ADR。继续自动推进 B–E、agent 互审，直到正式 `limae` 为 Rust、Python deprecated 且参考实现与测试保留；润色、两宿主 hook、正式入口切换尚未完成，实际 tag / release / publish 前停止。
