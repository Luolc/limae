...
fn render(lexicon: &Lexicon, ordered: &[&Entry]) -> String {
    ...
    page.push_str("<div class=\"page\"><nav><div class=\"toc-label\">目次</div>");
    for (index, entry) in ordered.iter().enumerate() {
        let _ = write!(
            page,
            "<a href=\"#w{index}\">{}<span class=\"sound\">{}</span></a>",
            escape(&entry.term),
            escape(&entry.pinyin.concat()),
        );
    }
    ...
    for (index, entry) in ordered.iter().enumerate() {
        ...
        for example in &entry.examples {
            let _ = write!(
                page,
                "<div class=\"eg\"><div class=\"before\"><span class=\"mark\">原</span>{}</div>\
