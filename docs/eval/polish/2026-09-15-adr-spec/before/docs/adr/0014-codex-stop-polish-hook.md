...
整份由 [ADR-0016](0016-hook-mechanical-only.md) 取代 (2026-09-11)：Codex 侧的 hook 放弃，`.codex/config.toml` 与对应的仓级契约随实现任务删除。理由正是本文「实测」一节记下的形状 —— `systemMessage` 只能追加在原回复下方、替换不了原文，一个只做排版修复的 hook 在那个位置没有意义。
