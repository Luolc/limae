...
本次基线是 `5640370c84745eeb365b91ecd25a91dcf63c443f`，下文的实现观察与测试盘点均截至 2026-09-06。已读取 ADR-0001 至 ADR-0014、[规则规范](../../spec/rules.md)、[fixture 合同](../../spec/README.md) 及全部 49 组共享 fixture。它们覆盖规则文本行为，但不覆盖完整 CLI、配置发现、文件系统和宿主协议。

Python 已实现 21 条确定性规则，包括实验规则；入口仍是 `limae [--fix] FILE...` / `--all`、`limae polish -` 和 `limae hook`。`check` / `format` 子命令、文件 polish 与结构保护器是 [ADR-0008](0008-limae-polish-cli.md) 的后续方向，不能算作已有的兼容面。规则与配置目前以 `spec/` 为准，旧 ADR 中的旧 id / 名字按 [ADR-0010](0010-rule-id-naming.md)、[ADR-0013](0013-drop-rename-transition-aliases.md) 理解。

## 决定

### 一、推荐范围与终态

推荐第一个里程碑交付**全部现有确定性 lint / fix 的可运行 Rust CLI**：包括配置、忽略、行内指令、实验规则及其词表，并先在开发工作树中显式调用。实验规则仍默认关闭，但不能把「关闭时没报错」当作移植完成。分发、polish、两种宿主 hook 依次验收；Python 在整个迁移期保留参考实现 (reference implementation)，其测试继续运行。
...
E 将 Python 的开发 / reference 命令显式命名为 `limae-python`，避免同名入口冲突；文档和包说明标记 deprecated，参考测试继续运行，不通过 stdout 弃用横幅破坏协议。仓内默认 hook / dogfood 转向已验收的 Rust；本机全局 CLI 安装与受管 hook 切换由 orchestra 通过 machine-setup PR / 既有安装流程协调，不手改家目录。外部消费仓只做必要调查与准备，不把整套跨项目迁移列为本任务前置条件，也不等待人工切换窗口。

本次不做 `contextual`、`quote_style`、新规则、语言探测、LSP、SDK、Python FFI、模型默认调优、文档级 lint、文件 / 多文件 polish。它们都不是 Rust 迁移的前置条件；进度只记 [tracker](../tracker.md)，本文任务编号只用于依赖和验收，不维护完成状态。

### 二、一个 package，显式源码路径

采用单 package、一个 library target 加一个薄 binary target；不建多 crate workspace。库负责规则与可测试的业务边界，binary 只组装配置 / 文件 / 输出。没有第二个需要独立发布或依赖隔离的组件时，workspace 只会增加 manifests、可见性和 lint 继承成本。
...
`Cargo.toml` 放在仓根，源码不挤进 Python 的 `src/limae/`，Rust 测试不混进 Python 的 `tests/`。C / D 到来时才增加 `rust/polish/`、`rust/hook/`。先用同步函数、`&str` / `Path` 和有界的逐文件处理，不为性能推测引入 Tokio、线程池或通用插件注册框架。

### 三、共享资源与必要依赖

推荐 Rust 用 `include_str!` 从根 `spec/wordlists/` **构建时嵌入**，C 再嵌入 `spec/polish/`。源码数据仍只放在 `spec/`，`resources.rs` 只负责引用和解析，不手抄词条或 prompt。库初始化时对嵌入数据做类型校验，失败返回具名错误，不能退回空词表；文件缺席则构建失败。[include_str! 的官方合同](https://doc.rust-lang.org/std/macro.include_str.html) 支持在编译时将相对源文件路径的 UTF-8 文件包含为字符串。

两种方式的用途不同：运行时文件适合用户在不重建程序时修改词表；构建时嵌入适合让词表随版本固定、只安装一个 binary。[ADR-0007 §二](0007-ai-tells-rule-family-and-wordlists.md#二词表放-specwordlists是规范的一部分) 没有开放用户词表增删接口，本次也未发现已授权的运行时编辑需求。为了这项尚无需求的能力引入资源目录发现、路径配置、资源安装与失配处理，成本不成立；因此不增加 `--data-dir`、share 目录或启动解压流程。
...
根 Cargo package 的 `include` 纳入 `rust/`、所需 `spec/`、README；根布局使资源包含在包内，验收 `cargo package --list`、解包构建和安装。[Cargo include 文档](https://doc.rust-lang.org/cargo/reference/manifest.html#the-exclude-and-include-fields) 只保证源码包内容；[cargo install 文档](https://doc.rust-lang.org/cargo/commands/cargo-install.html) 说明 executable 安装到 `bin`。因此 B 的验收要搬走安装后的 binary，在没有源码、Python、附属资源的 cwd 中实际运行实验规则，不能只看包清单。默认先支持源码 / Git 的 Cargo 安装与预编译 binary；本轮不发布或占名。E 的发布前清单核对名字、发布配置和许可证：2026-09-06 本仓根目录无 license 文件、`pyproject.toml` 无 license 声明，先列缺口，不自行选择许可证，也不阻塞本地实现。PyPI / npm / Homebrew 新分发面另案处理。

| 依赖 | 引入点与理由 |
| --- | --- |
| [regex](https://docs.rs/regex/latest/regex/) | Markdown / 句式匹配；标准库无正则。只保留容易表达的模式，look-around 改为显式邻接判定，不增加 PCRE / fancy-regex |
| [toml](https://docs.rs/toml/latest/toml/) | A5 的配置解析首次引入并独占 manifest / lock；A4 术语表与后续 fixture `.conf` 复用。先用 `Value` 校验已知键，不增加配置框架 |
| [ignore 的 GitignoreBuilder](https://docs.rs/ignore/latest/ignore/gitignore/struct.GitignoreBuilder.html) | 仅解析找到的 `.limae-ignore`；不启用文件 walker 隐含的 `.gitignore` / 全局排除。`add` 可能返回部分错误，必须处理；与 pathspec 的匹配差异由 A 的测试裁定 |
...
| 配置与忽略 | [config.py](../../src/limae/config.py) 从 cwd 向上，`.git` 文件与目录都终止；两种载体以最近的完整配置为准。CLI enable / disable 连空字符串参数也须按「flag 出现」处理，跳过配置读取；坏 pyproject 不能越过。未知的未实现键当前不读取，不能加 `deny_unknown_fields` 误拒 polish / quote_style。忽略发现独立，路径 resolve 后相对 ignore 根匹配，根外文件保留，输入顺序不变 |
...
这些读数是迁移线索，不在本 ADR 新立规则规范。常见路径默认按 Python 兼容；发现 Python 与 spec 不一致时，先提交独立的规范 / reference 修正并重定基线。不得通过忽略差异列表把结论改成「全部兼容」。

### 五、测试迁移映射与差分验收

Rust 使用 Cargo 内建测试框架：模块内 unit tests 覆盖局部业务边界，`rust/tests/integration.rs` 接入共享 golden 等集成模块，library 文档示例用 doctest。A1 即为首个可用 API 添加一条可执行文档示例，并从 `Doc-tests` 结果确认执行。[Cargo test 文档](https://doc.rust-lang.org/cargo/commands/cargo-test.html#target-selection) 的默认集合含 unit / integration / doctest，`--all-targets` 的显式集合不含 doctest，所以全量检查使用默认 `cargo test --locked`。缓存可用于减少重复构建，不引入 nextest 或额外测试框架。
...
| [conftest.py](../../tests/conftest.py) / [test_environment.py](../../tests/test_environment.py) | 清继承的 `LIMAE_*` 并主动注入变量作回归；Rust 测试为每个 child 配独立 env / cwd / 临时目录，避免并行测试修改进程级环境；保留「注入后运行真实用例」的有效对照 |
...
优先获得完整的确定性工具，再扩大分发和宿主影响面；代价是迁移期维护两套实现，Rust 词表更新需要重建 binary。换来的是单文件部署与更少的路径 / 安装状态。未测性能前不承诺加速倍数。规则事实始终留在 `spec/`，两实现的差异必须被测试暴露并逐项裁定。agent 自动推进与互审覆盖当前 Python 已实现功能，不把后续产品功能或其他消费仓的整体迁移加进完成条件。
