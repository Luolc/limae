...
规则 id 稳定且不复用，当前是 R1–R9；实现报告违规时用的 id 必须与这里一致。fixture 格式与 runner 的判定见 `spec/README.md`。
...
2. **R1** CJK 旁半角标点转全角
3. **R3** 半角括号外侧空格
4. **R4–R9** 各空格规则，按 id 次序

理由：宽度转换在前 (R2、R1)，空格规则在后 —— 转换的产物要落进空格规则的判定范围，如 R2 换出的半角括号由 R3 接着补空格。R3 到 R9 之间管的边界互不重叠 (括号、字母、数字、单位、定界反引号、破折号、链接各是不同的字符类)，相互顺序不影响结果，按 id 固定只为确定性。被关掉的规则跳过，其余次序不变。
