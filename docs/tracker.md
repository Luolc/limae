...
- **Rust 全量迁移**：[ADR-0015](adr/0015-rust-migration.md) 经 PR #66 合入，仓级约定 (G) 经 PR #67 合入；A1 文本基础与 Rust CI (#69)、A2 Markdown 保护范围 (#73)、A5 配置解析 (#75) 已完成，A2 前的引用配对基线修正见 #71。A3a 宽度规则 (zh-typography-1 / 2 / 10，#77) 与 A3b 正文间距 (3 / 4 / 5 / 6，#79) 已完成行检查与正文片段修复；规则 11 的非重叠计数、同起点报告顺序与合成样例已在 #81 补齐；A3c 结构间距原语 (#83) 与纯排版文档编排 (#85) 已完成；生产 API 的适用排版黄金样例完整通过，实验规则与行内指令分别归 A4 / A6。A4 首批五条词表规则与构建嵌入 (#87) 已完成，英文大小写与边界合同先由 #88 独立澄清；句式与造词原语 (#90) 已完成；下一批术语检查与片段修复，随后接文档实验规则编排，承接边界见 #90 PR 描述。按 ADR 的 A–E 自动推进、agent 互审，直到正式 `limae` 为 Rust、Python deprecated 且参考实现与测试保留；实际 tag / release / publish 前停止。进度仅在此记，任务依赖与验收合同见 ADR。
- **A4 / A6 黄金集交叉依赖**：`experimental-en-ai-tells`、`experimental-zh-ai-tells`、`experimental-zh-secret`、`experimental-zh-zero-noun` 各有一条生效的行内禁用指令，A4 文档集成时须保留这四组的 A6 依赖；`experimental-english-unicode` 没有这项依赖，不能过滤被指令影响的 finding 后声称完整通过；A6 后与全部黄金集一起验收 (见 [PR #91 事实复核](https://github.com/Luolc/limae/pull/91#issuecomment-5559435631))。
- **Rust 测试编排清理**：按 [PR #85 复审 P2](https://github.com/Luolc/limae/pull/85#issuecomment-5559083398)，后续触及规则测试时，删除测试内单轮片段编排，或改走生产 `Typography` API；三个规则族的 fixture 子集循环与动态黄金集重复，清理时保留原语业务断言与测试分辨力。
