...
§二、§三、§四、§五、§八 由 [ADR-0016](0016-hook-mechanical-only.md) 取代 (2026-09-11)：hook 不再调模型，每批就地跑确定性修复，跨批只带「在不在围栏里」一个布尔，A/B、编号、`Stop` 回注与台账全部停止。§一 (挂 `MessageDisplay`、display-only)、§六 (fail-open)、§七 (先本仓自试再交 machine-setup) 的内容由 ADR-0016 §一 接过并成为正本，本文不再是 hook 合同的正本。
