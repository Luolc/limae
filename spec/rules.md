...
4. **含假名的引用 span**：引用括号对 `「」`、`『』`、`《》` 的配对内容含假名 (ひらがな / カタカナ，U+3040–U+30FF) 时，这对括号之间的内容视同行内代码 —— 逐字引用的日文原文不是本规则集要管的行文，改写会毁掉引用的可追溯性。范围不跨行，只在行内代码之外识别；与 URL 范围重叠时以引用 span 为准 (整段引用原样保留)。
   - **配对**：同一行内，一个开括号与它右边最近的同类闭括号配对。没有闭括号的开括号不构成 span，照旧参与各规则判定。
   - **嵌套**：候选按开括号位置从左到右取，取到的那一对胜出 —— 即最外层的一对，落在它内部的配对不再单独识别 (不做更复杂的嵌套解析)。
...
「ひらがな『dotfile管理』の例」 → 不变 (取最外层的一对)
《Python编程》                 → 《Python 编程》 (不含假名，R4 照常)
...
理由：宽度转换在前 (R10、R2、R1)，空格规则在后 —— 转换的产物要落进空格规则的判定范围：R10 换出的半角数字接着被 R5 / R6 补空格，R2 换出的半角括号由 R3 接着补空格。R3 到 R9 之间管的边界互不重叠 (括号、字母、数字、单位、定界反引号、破折号、链接各是不同的字符类)，相互顺序不影响结果，按 id 固定只为确定性。R11 删空格，放在所有补空格的规则之后收尾 —— R1 把 `你好, 世界` 转成 `你好， 世界` 之后正是 R11 把这个空格删掉；R8 刚补的破折号旁空格不被回收，靠的是 R11 判定里的破折号豁免而不是顺序。被关掉的规则跳过，其余次序不变。

## 配置

规则分两类：**绝大多数规则默认启用**；个别规则默认关闭，并在自己的条目里标明 (当前只有 R9)。配置能关掉默认启用的规则、打开默认关闭的规则：关掉的规则既不报违规，`--fix` 也原样输出；打开的规则与默认启用的规则行为一致。所以「没有任何配置」永远等价于「默认集」(默认启用规则的全体)，各实现的默认行为一致，消费方不加配置就不会有行为变化。
...
1. **CLI flag `--disable` 与 `--enable`**：都接规则 id，可重复给也可逗号分隔 —— `--disable R3`、`--disable R1,R3`、`--disable R1 --disable R3` 等价于关掉列出的规则，`--enable` 同语法。命令行上只要出现了两者之一，配置文件整体不生效，缺席的另一个键按空算。
...
- **修复**：在边界处插入一个半角空格。
- **年月日不豁免**：`2011年5月15日` 这类中文计量单位紧跟数字的写法同样逐边界报；豁免与否是配置层未来的事，本规范的判定不带例外。
