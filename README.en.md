...
It also ships a [pre-commit](https://pre-commit.com/) hook, a `.limae-ignore` file, inline `<!-- limae-disable-next-line -->` comments, and a `limae polish` subcommand that hands prose to a locally logged-in LLM CLI — from stdin, or over repository files behind an explicit per-call `--share-repo-with-engine` flag (the engine is started in a copy of the tracked files, reads what it finds there and sends it to its service; limae's own write-back touches only the named files and skips a file whose rewrite has a different number of headings, code fences or list items, while the engine itself and what you configured under HOME can still write elsewhere). The flag's `--help` and the Chinese README say what that does and does not protect.

For agents there is a skill in [`skills/write-naturally/`](https://github.com/Luolc/limae/tree/main/skills/write-naturally) ([agentskills.io](https://agentskills.io/specification) format): copy the directory into your agent's skill directory and the model reads the Chinese guide and the lexicon before it writes, instead of having its text polished afterwards.
